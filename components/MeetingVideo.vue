...
const { videoRefs, isLocalAudioEnabled, isLocalVideoEnabled, isConnected, peers, joinRoom, leaveRoom, toggleAudio, toggleVideo, dominantSpeaker, peersWithAudioStatus } = useHms()
	...
	<div class="container mx-auto mb-8 mt-8" data-testid="meeting-container">
		<div v-if="isConnected" class="conference-section" data-testid="conference-section">
			<div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
				<div v-for="peerWithAudioStatus in peersWithAudioStatus" :key="peerWithAudioStatus.peer.id"
					:class="{ 'dominant-speaker': dominantSpeaker && peerWithAudioStatus.peer.id === dominantSpeaker.id, 'relative w-full rounded-lg bg-zinc-500 sm:w-auto': true, }">
					...
					<div
						class="peer-name absolute bottom-2 left-2 z-[2] mt-2 rounded bg-[rgb(0,0,0,0.3)] px-2 text-white flex items-center">
						<p>{{ peerWithAudioStatus.peer.name }}</p>
						<MicrophoneIcon v-if="peerWithAudioStatus.isAudioEnabled" class="h-4 w-4 ml-2 text-green-500" />
						<svg v-else class="h-4 w-4 ml-2 text-red-500" xmlns="http://www.w3.org/2000/svg" fill="none"
